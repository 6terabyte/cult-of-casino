import { Resolver, Query, InputType, ArgsType, Field, Arg, Ctx, Mutation, UseMiddleware, Int } from 'type-graphql';
...
export class UserResolver {
  ...
  */
  ...
  // customer使う
  @Query(returns => UserEntity, { nullable: true })
  async getUserBySession(@Arg("session") session: string): Promise<UserEntity | undefined> {
    return UserEntity.findOneBy({session, using: true});
  }

  // ログインなしでアクセス可
  @Query(returns => UserEntity, { nullable: true })
  async getUserByAuth(@Arg("auth", { validate: false }) authInfo: AuthInfo): Promise<UserEntity | undefined> {
    const userData = await UserEntity.findOneBy({email: authInfo.email, using: true});
    if (!userData) return
  ...
  async addUser(@Arg("data", { validate: false }) newUser: AddUser): Promise<UserEntity> {
    if(!(newUser.emailChangeTemp && newUser.hash && newUser.name && newUser.screenName)) {
      return undefined
    }
  ...
  // userrouterからしか触らない想定
  @Query(returns => UserEntity, { nullable: true })
  async authSuccess(@Arg("data") editData: AuthSuccess): Promise<UserEntity | undefined> {
